 ...
 * @author Daniel
 */
@WebServlet(name = "AtualizaVendaPagamento", urlPatterns = {"/loja_server/AtualizaVendaPagamento"}, initParams = {
    @WebInitParam(name = "venda", value = ""),
    @WebInitParam(name = "nomeUsuario", value = ""),
    @WebInitParam(name = "senha", value = "")})
public class AtulizarVendaPagamento extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       String usuario = new String(request.getParameter("nomeUsuario").getBytes("iso-8859-1"), "UTF-8");
        ...
        if (cod > 0) {
            ...
            response.setHeader("sucesso", pro.atualizaVendaPagamento(c,cod));
